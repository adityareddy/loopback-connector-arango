...
generateArangoURL = function(options) {
  ...
  if (username && options.password) {
    return "http://" + username + ":" + options.password + "@" + options.hostname +
...
ArangoDB = (function() {
  ...
  ArangoDB.evaluateWhere = function(where, resultVariable, model) {
    ...
    for (condProp in where) {
      condValue = where[condProp].toString();
      if (condProp === 'id') {
        condProp = '_id';
  ...
  ArangoDB.prototype.automigrate = function(numberOfShards, replicationFactor, callback) {
    var dropAndCreate;
